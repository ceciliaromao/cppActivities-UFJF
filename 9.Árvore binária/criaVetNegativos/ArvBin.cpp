...
int* ArvBin::criaVetNegativos(int k, int *n)
{
    ...
    *n = pow(2, k);
    int *vet = new int[*n];
    int pos = 0;
    auxCriaVetNegativos(raiz, k, 0, vet, &pos);
    for(int i = pos; i < *n; i++)
        vet[i] = -1;
    return vet;
}
 
void ArvBin::auxCriaVetNegativos(NoArv *p, int k, int atual, int vet[], int *pos)
{
    if(p != NULL)
    {
        if(atual == k && p->getInfo() < 0)
            vet[(*pos)++] = p->getInfo();
        if(atual < k)
        {
            auxCriaVetNegativos(p->getEsq(), k, atual+1, vet, pos);
            auxCriaVetNegativos(p->getDir(), k, atual+1, vet, pos);
        }
    }
}
